...
function SignupPage() {
  ...
  const handleSignUp = async (e) => {
    ...
    if (data) {
      setMassage("User account created!")
    }
    setEmail("")
    setPassword("")
    setFirstName("")
      ...
      <div className="flex w-full flex-col justify-center lg:w-1/2 lg:px-12 text-white">
        <div className="mx-auto w-full max-w-lg sm:max-w-3xl rounded-xl bg-[#1a1a1a] p-8 shadow-lg sm:border-0 sm:bg-transparent sm:shadow-none lg:border-0 lg:bg-transparent lg:shadow-none">
          ...
          <form className="space-y-4" onSubmit={handleSignUp}>
            ...
            <label htmlFor="terms" className="flex items-center space-x-2 text-sm text-gray-300">
              <input
                type="checkbox"
                id="terms"
                checked={agreed}
                onChange={(e) => setAgreed(e.target.checked)}
                className="rounded border-gray-600 bg-transparent text-teal-400 focus:ring-teal-400"
              />
              <span>
                I agree to the{" "}
            ...
            <div className="relative my-6">
              ...
              <div className="relative flex justify-center text-sm">
                <span className="bg-[#1a1a1a] px-3 text-gray-300 sm:bg-[#141414]">
                  Or register with
                </span>
              </div>
            </div>
...
// Icon Components (same as before, just made accessible)
